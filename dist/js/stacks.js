...
(function () {
  ...
  Stacks.addController("s-table", {
    ...
    sort: function (evt) {
      ...
      var tbody = table.tBodies[0]; // the column slot number of the clicked header
      ...
      var slotIndex = buildIndex(tbody); // the default behavior when clicking a header is to sort by this column in ascending
      // direction, *unless* it is already sorted that way

      var direction = this.getElementData(colHead, "sort-direction") === "asc" ? -1 : 1;
      var rows = Array.from(table.tBodies[0].rows); // if this is still false after traversing the data, that means all values are integers (or empty)
      // and thus we'll sort numerically.

      var anyNonInt = false; // data will be a list of tuples [value, rowNum], where value is what we're sorting by
  ...
  function buildIndexOrGetCellSlot(section, findCell) {
    var index = [];
    var curRow = section.children[0]; // the elements of these two arrays are synchronized; the first array contains table cell elements,
    ...
    var growing = [];
    var growingRowsLeft = []; // continue while we have actual <tr>'s left *or* we still have rowspan'ed elements that aren't done

    while (curRow || growingRowsLeft.some(function (e) {
      return e;
    })) {
      var curIndexRow = [];
...
(function () {
  ...
  Stacks.addController("s-magic", {
    abracadabra: function (evt) {
